server <- function(input, output) {
  ...
  output$Plot_StepTimes1 <-  renderPlot({
    Plot_StepTimes1 <-  tab_Collateddata_1() %>% dplyr::select(`ID`, `Step Time Step 1`, `Step Time Step 2`, `Step Time Step 3`, `Step Time Step 4`, 
                                                               `Step Time Step 5`, `Step Time Step 6`, `Step Time Step 7`, `Step Time Step 8`,
                                                               `Step Time Step 9`, `Step Time Step 10`, `Step Time Step 11`, `Step Time Step 12`,
                                                               `Step Time Step 13`)
    ...
    Plot_StepTimes1 <-  Plot_StepTimes1 %>% reshape2::melt(id = c("ID"))
  ...
  output$Plot_ContactTimes1 <-  renderPlot({
    Plot_ContactTimes1 <-  tab_Collateddata_1() %>% dplyr::select(`ID`, `Contact Time last`,  `Contact Time 2nd last`, `Contact Time 3rd last`,) 
    
    all_na <- function(x) any(!is.na(x))
    Plot_ContactTimes1 <- Plot_ContactTimes1 %>% select_if(all_na) %>% reshape2::melt(id = c("ID"))
  ...
  output$Plot_Cadencev11 <-  renderPlot({
    Plot_Cadencev11 <-  tab_Collateddata_1() %>% dplyr::select(`ID`, `Cadencev1 Step 1`, `Cadencev1 Step 2`, `Cadencev1 Step 3`, `Cadencev1 Step 4`, `Cadencev1 Step 5`, `Cadencev1 Step 6`, `Cadencev1 Step 7`, `Cadencev1 Step 8`, `Cadencev1 Step 9`, `Cadencev1 Step 10`, `Cadencev1 Step 11`, `Cadencev1 Step 12`, `Cadencev1 Step 13`) 
    
    all_na <- function(x) any(!is.na(x))
    Plot_Cadencev11 <- Plot_Cadencev11 %>% select_if(all_na) %>%  reshape2::melt(id = c("ID"))
    
    
    ggplot(data=Plot_Cadencev11, aes(x=variable, y=value, color=`ID`), group=1) + geom_line(aes(group=`ID`)) +
  ...
  output$Plot_Cadencev21 <-  renderPlot({
    Plot_Cadencev21 <-  tab_Collateddata_1() %>% dplyr::select(`ID`, `Cadencev2 Step 1`, `Cadencev2 Step 2`, `Cadencev2 Step 3`, `Cadencev2 Step 4`, `Cadencev2 Step 5`, `Cadencev2 Step 6`, `Cadencev2 Step 7`, `Cadencev2 Step 8`, `Cadencev2 Step 9`, `Cadencev2 Step 10`, `Cadencev2 Step 11`, `Cadencev2 Step 12`, `Cadencev2 Step 13`) 
    
    all_na <- function(x) any(!is.na(x))
    Plot_Cadencev21 <- Plot_Cadencev21 %>% select_if(all_na) %>% reshape2::melt(id = c("ID"))
